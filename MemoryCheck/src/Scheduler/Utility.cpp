...
void printStackTrace(char **stackSymbols, size_t sizeOfStacktrace) {
    ...
    for (int i = 0; i < sizeOfStacktrace; i++) {
        ...
        functionNameEnd = currentStackSymbol.find_first_of('+', functionNameBegin) - 1;
        returnOffsetBegin = functionNameEnd + 2;
        ...
        std::cerr << "  " << binaryName << "(" << demangleFunctionName(mangledFunctionName) << "+"
                  << returnAddressOffset << ")"
...
char *demangleFunctionName(std::string mangledFunctionName) {
    ...
    //      use status better + do not return the mangledFunctionName
    free(functionName);
    if (status == 0)
        return ret;
    return NULL;
