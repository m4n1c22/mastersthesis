...
std::string const TRACE_IN = TRACE_PREFIX + "bigshot/trace01.gv";

TEST_CASE("instructed bigshot runs from command line") {
    REQUIRE(isFile(TRACE_IN));
    ensureLoggerInitialized();

    SECTION("without input or output trace") {
        int exitStatus = WEXITSTATUS(std::system("timeout 2 ../bin/bigshot_irs -v"));
        CHECK(exitStatus == 0);
    }

    SECTION("without input trace and with output trace") {
        int exitStatus = WEXITSTATUS(
                std::system(("timeout 2 ../bin/bigshot_irs -v -o" + TRACE_OUT).c_str()));
        CHECK(exitStatus == 0);
    ...
    SECTION("with input trace01") {
        int exitStatus = WEXITSTATUS(std::system(
                ("timeout 2 ../bin/bigshot_irs -v -i" + TRACE_IN + " -o" + TRACE_OUT).c_str()));
        CHECK(exitStatus == 0);
        REQUIRE(isFile(TRACE_OUT));
    ...
    SECTION("for trace01 with optimizing scheduler") {
        for (int j = 0; j < 10; ++j) {
            int exitStatus = WEXITSTATUS(
                    std::system(("timeout 2 ../bin/bigshot_irs -v -i" + TRACE_IN).c_str()));
...
TEST_CASE("plain bigshot runs from command line") {
    REQUIRE(!isFile(TMP));

    int exitStatus = WEXITSTATUS(std::system(("timeout 2 ../bin/bigshot_plain > " + TMP).c_str()));
    CHECK(exitStatus == 0);
