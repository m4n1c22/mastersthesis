...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
    ...
    nThreads = atoi(bs_argv[1]);
    char *inputFile = bs_argv[2];
    char *outputFile = bs_argv[3];
    ...
    if (nThreads > numOptions) {
        printf("WARNING: Not enough work, reducing number of threads to match number of "
               "options.\n");
        nThreads = numOptions;
    }
...
#define LINESIZE 64
    ...
    for (i = 0; i < nThreads; i++) {
        tids[i] = i;
        {
            ...
            pthread_create(&_M4_threadsTable[_M4_i], NULL, (void *(*)(void *))bs_thread,
                           (void *)&tids[i]);
