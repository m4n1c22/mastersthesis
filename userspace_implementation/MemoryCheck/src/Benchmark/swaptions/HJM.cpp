...
int HJM_Factors(FTYPE **ppdFactors, // Output matrix that stores factor volatilities for different
                                    // maturities
                int iN, int iFactors,
                ...
                FTYPE **ppdFacBreak) // Input matrix of factor weights for each maturity
{
    ...
    // For instance, the user may supply				   Maturity:	1	   2
    // 3 4  total vol (pdVol) as Sigma:  1.35%, 1.30%, 1.25%, 1.20%,....  and the weight breakdown
    // (ppdFacBreak) as Factor 1:   0.55,  0.60,  0.65,  0.69,....
    //												   Factor
    // 2:  0.44,  0.39,  0.34,  0.30,....
    //												   Factor
    // 3:  0.01,  0.01,  0.01,  0.01,....  Note that the weights add up to 1 in each case. Also, the
    // weights are
    // based
    // on
    ...
    // each maturity.  The output (ppdFactors) may look something like: Maturity:	1	   2
    // 3
    // 4
    //												   Factor
    // 1:  1.00%  1.00%  1.00%  1.00%
    //												   Factor
    // 2:  0.90%  0.82%  0.74%  0.67%
    //												   Factor
    // 3:  0.10%  0.08%  0.05%  0.03%
...
int HJM_Drifts(FTYPE *pdTotalDrift, // Output vector that stores the total drift correction for each
                                    // maturity
               FTYPE **ppdDrifts, // Output matrix that stores drift correction for each factor for
                                  // each maturity
        ...
        long *lRndSeed)      // Random number seed
{
    ...
    for (j = 1; j <= iN - 1; ++j) {
        ...
        for (l = 0; l <= iFactors - 1; ++l)
            pdZ[l] = CumNormalInv(
                    RanUnif(lRndSeed)); // shocks to hit various factors for forward curve at t
