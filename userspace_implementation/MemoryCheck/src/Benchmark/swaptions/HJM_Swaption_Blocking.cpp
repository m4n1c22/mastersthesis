...
int HJM_Swaption_Blocking(
        ...
        FTYPE dPaymentInterval, // frequency of swap payments e.g. dPaymentInterval = 0.5 implies a
                                // swap payment every half
        // year
        // HJM Framework Parameters (please refer HJM.cpp for explanation of variables and
        // functions)
...

{
    ...
    FTYPE dStrikeCont; // Strike quoted in continuous compounding convention.
                       // As HJM rates are continuous, the K in max(R-K,0) will be dStrikeCont and
                       // not dStrike.
    if (dCompounding == 0) {
        dStrikeCont = dStrike; // by convention, dCompounding = 0 means that the strike entered by
                               // user has been quoted  using continuous compounding convention
    } else {
    ...
    int iSwapVectorLength; // Length of the HJM rate path at the time index corresponding to
                           // swaption maturity.
    ...
    //==================================
    // **** per Trial data **** //
    FTYPE *pdDiscountingRatePath; // vector to store rate path along which the swaption payoff will
                                  // be discounted
    FTYPE *pdPayoffDiscountFactors; // vector to store discount factors for the rate path along
                                    // which the swaption
    // payoff will be discounted
    FTYPE *pdSwapRatePath; // vector to store the rate path along which the swap payments made will
                           // be discounted
    ...
    for (i = iFreqRatio; i <= iSwapTimePoints; i += iFreqRatio) {
        if (i != iSwapTimePoints)
            pdSwapPayoffs[i] = exp(dStrikeCont * dPaymentInterval) -
                               1; // the bond pays coupon equal to this amount
        if (i == iSwapTimePoints)
            pdSwapPayoffs[i] = exp(
                    dStrikeCont *
    ...
    for (l = 0; l <= lTrials - 1; l += BLOCKSIZE) {
        ...
        iSuccess = Discount_Factors_Blocking(pdPayoffDiscountFactors, iN, dYears,
        ...
        iSuccess = Discount_Factors_Blocking(pdSwapDiscountFactors, iSwapVectorLength,
                                             dSwapVectorYears, pdSwapRatePath, BLOCKSIZE);
        if (iSuccess != 1)
            return iSuccess;
    ...
    //!!! WARNING: added absolute numinator to prevent negative sqrt
    dSimSwaptionStdError = sqrt((std::abs(dSumSquareSimSwaptionPrice -
                                          dSumSimSwaptionPrice * dSumSimSwaptionPrice) /
                                 lTrials) /
                                (lTrials - 1.0)) /
                           sqrt((FTYPE)lTrials);
