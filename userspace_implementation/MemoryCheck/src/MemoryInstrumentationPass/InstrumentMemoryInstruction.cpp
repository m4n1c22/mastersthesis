...
bool InstrumentMemoryInstruction::runOnBasicBlock(Function::iterator &BB) {
    for (BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI) {
        ...
        // %1 = cmpxchg i32* @glob_a, i32 0, i32 %2 seq_cst seq_cst
            ...
            if (Constant *constant = dyn_cast<Constant>(GEP->getOperand(0))) {
                std::string con_str;
                llvm::raw_string_ostream rso(con_str);
                constant->stripPointerCasts()->print(rso);
                std::string name = con_str.substr(1, con_str.find_first_of(' ', 0) - 1);
                annotated = checkAnnotatedVar(name);
            } else {
                annotated = checkAnnotatedVar(GEP->getOperand(0)->getName());
            }
            if (annotated) {
                Value *accessSize;
                // run def-use chain
                    ...
                    if (Instruction *inst = dyn_cast<Instruction>(U)) {
                        if (CallInst *CI = dyn_cast<CallInst>(U)) {
                            accessSize = ConstantInt::get(
                                    SizeTy,
                                    m_dataLayout->getTypeStoreSize(CI->getOperand(0)->getType()));
                            instrument(CI->getOperand(0), accessSize, CI, true);
