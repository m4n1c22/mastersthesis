...
#check if the log file exists or not.
if [ -f prod_cons_trace_0_proto_1.dat ]
then
	sudo rm prod_cons_trace_0_proto_1.dat
fi
if [ -f prod_cons_trace_1_proto_1.dat ]
then
	sudo rm prod_cons_trace_1_proto_1.dat
fi
if [ -f prod_cons_trace_2_proto_1.dat ]
then
	sudo rm prod_cons_trace_2_proto_1.dat
fi
if [ -f prod_cons_trace_3_proto_1.dat ]
then
	sudo rm prod_cons_trace_3_proto_1.dat
fi
sudo make test_prod_cons
#Execute the prog binary file for test runs times. And write the output in the overall log file.
while [ "$count" -lt "$test_runs" ]
do		
	sudo make run_prod_cons
	count=`expr $count + 1`
done
make unload
make cleanall
