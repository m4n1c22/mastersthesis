trace_prefix = ['_trace_0', '_trace_1', '_trace_2', '_trace_3']
...
def avg_exec_times_traces_irs(num_traces=4):
	for i in range(num_traces):
		num_runs = 0
		sum = 0.0
		filename = "last_zero_irs" + trace_prefix[i] + ".dat"		
		f = open(filename, "r")
		for line in f:
			num_runs += 1
			sum += float(line)
...
def avg_exec_time_traces(num_traces=4, num_protos=6):
	for i in range(num_protos):
		for j in range(num_traces):
			num_runs = 0
			sum = 0.0
			filename = proto_locs[i] + "last_zero"+ trace_prefix[j] + proto_postfix[i] + ".dat"		
			f = open(filename, "r")
			for line in f:
				num_runs += 1
				sum += float(line)
