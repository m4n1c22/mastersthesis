...
std::string const TRACE_IN = TRACE_PREFIX + "shared_pointer/trace01.gv";

TEST_CASE("shared_pointer with IRS runs from command line") {
    REQUIRE(isFile(TRACE_IN));
    ensureLoggerInitialized();

    SECTION("without input or output trace") {
        int exitStatus = WEXITSTATUS(std::system("timeout 2 ../bin/shared_pointer_irs -t 4 -v"));
        CHECK(exitStatus == 0);
    }

    SECTION("without input trace") {
        int exitStatus = WEXITSTATUS(std::system(
                ("timeout 2 ../bin/shared_pointer_irs -t 4 -v -o" + TRACE_OUT).c_str()));
        CHECK(exitStatus == 0);
    ...
    SECTION("with input trace01") {
        int exitStatus = WEXITSTATUS(std::system(
                ("timeout 2 ../bin/shared_pointer_irs -t 4 -v -i" + TRACE_IN + " -o" + TRACE_OUT)
                        .c_str()));
        CHECK(exitStatus == 0);
        REQUIRE(isFile(TRACE_OUT));
    ...
    SECTION("for trace01 with optimizing scheduler") {
        for (int j = 0; j < 10; ++j) {
            int exitStatus = WEXITSTATUS(
                    std::system(("timeout 2 ../bin/shared_pointer_irs -v -i" + TRACE_IN).c_str()));
...
TEST_CASE("Plain benchmark runs from command line") {
    ...
    int exitStatus = WEXITSTATUS(
            std::system(("timeout 2 ../bin/shared_pointer_plain -t 4 > " + TMP).c_str()));
    CHECK(exitStatus == 0);
